...
The screenshot command does not create or update Playwright golden baselines. To capture a different small set, add `--screenshots` to any audit command.
...
- `src/forensics.mjs` captures rectangles, computed layout properties, scroll geometry, tier classes, selected state, required-control availability, page overflow, and console exceptions. Approved dense analytical scrollers are identified separately from document-level overflow.
- `src/audit.mjs` sweeps widths in descending order, builds compact signatures, flags abrupt transitions for human review, correlates them with source boundaries, and optionally enriches changes with matched-rule data from Chrome DevTools Protocol.
